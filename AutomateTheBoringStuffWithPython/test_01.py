...
生成器函数
========
用关键字 yield 来返回值的函数，被称为生成器函数。
生成器函数被调用的时候，会返回一个生成器对象，它本质上还是一个迭代器，
因此它有和迭代器一样的特性，唯一的区别在于实现方式上不一样，生成器更加简洁。
yield 一次返回一个结果，在每个结果中间，挂起函数的状态，以便下次从它离开
的地方继续执行
"""


def f_fib(count):  # 定义一个生成器函数
    fib_n, fib_a, fib_b = 0, 0, 1
    while fib_n < count:
        print('yield 即将执行。。。。。。')
        yield fib_a  # 所以，这是一个生成器函数
        print('yield 已经执行！')
...
# 为什么要使用生成器？
# 显然，生成器逼格比迭代器高，它没有那么多冗长的代码，而且性能同样高效。

"""
==========
生成器表达式
==========
器表达式与列表推导式长的非常像，但是它俩返回的对象不一样，
前者返回生成器对象，后者返回列表对象。
"""
print('--------------------------------------')
print('列表')
...
"""
本质上而言，生成器保存的是算法，计算斐波那契数列的例子中，始终只有
n, a, b 这三个变量，每次调用这个函数就调用 __next__ 计算出下一个元素的值，
直到计算出最后一个元素，然后抛出 StopIteration 异常。

普通函数会循环全部次数，再接着往下执行

generator 函数被显示或隐式调用 __next__ 的时候被执行，遇到 yield
语句的时候返回，直到下次调用 __next__ 的时候，返回到 yield 语句处
继续执行 yield 语句后的代码，进入下一个循环，再次来到 yield 语句处返回。

所以，循环会被 yield 语句切开，并被 __next__ 展开。

我们通常不会调用 __next__ 而是用 for 循环来迭代

简单的情况，可以使用 生成器列表，复杂情况下可以使用 生成器函数
"""
...
　　（2）生成器是可以迭代的，但是只可以读取它一次。因为用的时候才生成，比如 a = (x*x for x in range(3))。
        !!!!注意这里是小括号而不是方括号。

　　（3）生成器（generator）能够迭代的关键是他有 __next__ 方法，工作原理就是通过重复调用 __next__ 方法，直到捕获一个异常。

　　（4）带有 yield 的函数不再是一个普通的函数，而是一个生成器 generator，可用于迭代

　　（5）yield 是一个类似 return 的关键字，迭代一次遇到 yield 的时候就返回 yield 右面的值。
        而且下一次迭代的时候，从上一次迭代遇到的 yield 后面的代码开始执行

　　（6）yield 就是 return 它返回的一个值，并且记住这个返回的位置。下一次迭代就从这个位置开始。

　　（7）带有 yield 的函数不仅仅是只用于 for 循环，而且可用于某个函数的参数，只要这个函数的参数也允许迭代参数。

　　（8）send() 和 next() 的区别就在于 send 可传递参数给 yield 表达式，这时候传递的参数就会作为 yield 表达式的值，
        而 yield 的参数是返回给调用者的值，也就是说 send 可以强行修改上一个 yield 表达式值。

　　（9）send() 和 next() 都有返回值，他们的返回值是当前迭代遇到的 yield 的时候，yield 右边表达式的值，其实就是当前迭代 yield 右边的参数。
